...
impl<'a> SimplePacket<'a> {
    pub fn sequence(&self) -> u64 {
        match self {
            SimplePacket::SmallReliable { sequence, .. }
            | SimplePacket::SmallUnreliable { sequence, .. }
            | SimplePacket::UnreliableSlice { sequence, .. }
            | SimplePacket::ReliableSlice { sequence, .. }
            | SimplePacket::Ack { sequence, .. } => *sequence,
        }
    }
    ...
    pub fn to_bytes(&self, b: &mut octets::OctetsMut) -> Result<usize, SerializationError> {
        ...
        match self {
            ...
            SimplePacket::Ack { sequence, ack_ranges } => {
                b.put_u8(4)?;
                b.put_varint(*sequence)?;
